...
|---|---|---:|---:|---:|---|
| template (most-owned legal fifteen) | model − form | 37 | -1.78 | 0.89 | no |
| template (most-owned legal fifteen) | model − priorSeason | 37 | -0.84 | 0.83 | no |
| random #1 (seed 20260827) | model − form | 37 | +0.16 | 0.24 | no |
| random #1 (seed 20260827) | model − priorSeason | 37 | +1.51 | 0.65 | **yes** |
| random #2 (seed 20260827) | model − form | 37 | -0.32 | 0.67 | no |
| random #2 (seed 20260827) | model − priorSeason | 37 | -1.30 | 0.65 | no |
| random #3 (seed 20260827) | model − form | 37 | -0.81 | 0.72 | no |
| random #3 (seed 20260827) | model − priorSeason | 37 | -0.89 | 0.88 | no |
| random #4 (seed 20260827) | model − form | 37 | -0.32 | 0.69 | no |
| random #4 (seed 20260827) | model − priorSeason | 37 | -0.70 | 0.78 | no |


**Nothing here separates the predictors.** Not one model-versus-`form` comparison clears two standard errors, and the sign of the difference flips across squads (1 of 5 positive). **This is a null result and it is reported as one** — the model does not make measurably better XI and captain decisions than `form` over one season, on any of these fifteens.
...
|---|---|---:|---:|---:|---:|---:|
| no-transfer | model | 37 | **1484** | 0 | 0 | £98.1m |
...
| greedy-1ft | model | 37 | **1875** | 37 | 0 | £96.2m |
| greedy-1ft | form | 37 | **1814** | 37 | 0 | £97.7m |
| greedy-1ft | priorSeason | 37 | **1148** | 2 | 0 | £97.5m |
| greedy-1ft | template (crowd proxy) | 37 | **1873** | 37 | 0 | £97.7m |

### Is the difference bigger than the noise?

| Policy | comparison | rounds | mean difference | ± s.e. | clears noise |
|---|---|---:|---:|---:|---|
| no-transfer | model − form | 37 | +8.43 | 2.68 | **yes** |
| no-transfer | model − priorSeason | 37 | +9.54 | 2.59 | **yes** |
| greedy-1ft | model − form | 37 | +1.65 | 2.98 | no |
| greedy-1ft | model − priorSeason | 37 | +19.65 | 2.65 | **yes** |

### What the simulated season says

**Held all season, the model's opening fifteen is worth 1484 points against 1172** — a gap of 312 over the season, which clears the noise floor comfortably. This is the ordering advantage from the section above, showing up exactly where Phase 2 predicted it would: **in which fifteen you own, not in how you arrange a fifteen you already have.** Note what the `form` row actually is — form cannot pick an opening squad, so that squad was chosen by last season's points per 90.

**Give both a transfer a week and most of that gap closes.** `form` goes from 1172 to 1814; the model goes from 1484 to 1875, a remaining gap of **61** which does **not** clear the noise floor. A weekly transfer is a powerful error-correction mechanism, and it corrects a weak opening squad faster than it improves a strong one. **A model that is better only before the first deadline is worth much less than the season totals first suggest.**
